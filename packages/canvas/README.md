...
사용자는 입력값에서 `data.local`, `data.global` 내에 있는 변수를 사용할 수 있습니다. 가령 `data.local`에 `test`라는 변수로 `1`이란 값이 대입되어 있다면, 아래와 같이 템플릿을 쓸 수 있습니다.

```javascript
{{ test }} + 2 // 3
```

또는 게임을 만들 경우, 사용자의 이름을 입력받고 싶을 것입니다. 이 때, 사용자의 이름이 `data.local.userName`에 저장되었다면, 이렇게 쓸 수 있습니다.
...
가령 사용자의 이름을 받는 경우, `data.local.userName`에 값을 대입한다고 합시다. 이 때 사용자가 악의를 품고, 아래와 같이 `실행 가능한 문법` 문자열을 삽입하면 문제가 됩니다.
